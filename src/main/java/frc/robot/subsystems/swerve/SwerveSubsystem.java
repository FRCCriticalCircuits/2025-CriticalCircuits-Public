...
public class SwerveSubsystem extends SubsystemBase {
    ...
    // SwerveSetpointGenerator(config);
    ...
    private final StructPublisher<Pose2d> estimateFieldPublisher;
    private final StructPublisher<Pose2d> visionEstimatePublisher;

    public static SwerveDriveSimulation driveSimulation;

    public static final Lock odometryLock = new ReentrantLock();

    // 254 setpoint gen for kinematic constraints
    private final SwerveSetpointGenerator setpointGenerator;
    private SwerveSetpoint pSwerveSetpoint;
    ...
    private SwerveSubsystem() {
        ...
        setpointGenerator = new SwerveSetpointGenerator(config,
                Units.rotationsToRadians(10));

        // Initialize the previous setpoint to the robot's current speeds & module
        // states
        ChassisSpeeds curSpeeds = getChassisSpeeds();
        SwerveModuleState[] curModuleStates = getSwerveModuleStates();
        pSwerveSetpoint = new SwerveSetpoint(curSpeeds, curModuleStates, DriveFeedforwards.zeros(config.numModules));
        ...
        // Telemetry
        currentSwerveStatePublisher = NetworkTableInstance.getDefault()
                .getStructArrayTopic("/SwerveStates/CurrentState", SwerveModuleState.struct).publish();
        desireSwerveStatePublisher = NetworkTableInstance.getDefault()
                .getStructArrayTopic("/SwerveStates/DesiredState", SwerveModuleState.struct).publish();
        estimateFieldPublisher = NetworkTableInstance.getDefault()
                .getStructTopic("/PoseEstimate/estimatedField", Pose2d.struct).publish();
        visionEstimatePublisher = NetworkTableInstance.getDefault()
                .getStructTopic("/PoseEstimate/visionField", Pose2d.struct).publish();
    }
     ...
     */
    public synchronized void updatePoseEstimator(Pose2d visionEstimatedPose, double timeStampSeconds,
    ...
    public void driveRobotRelative(ChassisSpeeds speeds) {
        ...
        pSwerveSetpoint = setpointGenerator.generateSetpoint(
                pSwerveSetpoint, // The previous setpoint
                speeds, // The desired target speeds
                0.02 // The loop time of the robot code, in seconds
        );
        setModuleStates(pSwerveSetpoint.moduleStates()); // Method that will drive the robot given target module states
    }
